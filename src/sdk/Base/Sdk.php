...
class Sdk extends Object
{
     ...
     * @var string
     */
    public static $cacheKey = 'Sdk::findFile()';
    ...
    public function init()
    {
        ...
        {
            Sdk::$environment = constant('Sdk::' . strtoupper($_SERVER['SDK_ENV']));
        }
        // 运行环境配置
        Sdk::$environment = Sdk::DEVELOPMENT;
     ...
     * 加载模块
     *
     *     self::modules(['modules/foo', MODPATH.'bar']);
     *
     * @param   array $modules 模块列表和路径
     *
    ...
    public function modules($modules = null)
    {
        ...
        // 应用目录最先搜索
        if ( ! in_array(APPPATH, self::$_paths))
        {
            array_unshift(self::$_paths, APPPATH);
        }
        ...
        foreach ($modules as $name)
        {
            ...
            if (is_dir($path))
            {
                self::$_paths[] = self::$_modules[$name] = realpath($path) . DIRECTORY_SEPARATOR;

                // 加载的同时，就载入初始化文件
                $init = self::$_modules[$name] . 'init' . EXT;
                if (is_file($init))
                {
                    require_once $init;
                }
            ...
            {
                throw new BaseException("Attempted to load an invalid or missing module ':module' at ':path'", [
                    ':module' => $name,
        ...
        // 框架目录永远放在最后
        if (in_array(SYSPATH, self::$_paths))
        {
            // 已经存在啦，那就删除已经存在的，再次放到最后
            $sysPathKey = array_search(SYSPATH, self::$_paths);
            unset(self::$_paths[$sysPathKey]);
        }
     ...
     *     Sdk::findFile('views', 'template');
     *
     *     // 返回media/css/style.css的绝对路径
     *     Sdk::findFile('media', 'css/style', 'css');
     *
     *     // 返回合并后的数组
     *     Sdk::findFile('config', 'main');
     *
