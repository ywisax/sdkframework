...
class Validation implements ArrayAccess
{
    ...
    public static $validHelperClass = 'sdk\Security\Valid';
    ...
    public function check()
    {
        ...
        foreach ($expected as $field)
        {
            ...
            $data[$field] = Arr::get((array) $this, $field);
        ...
        foreach ($rules as $field => $set)
        {
            ...
            foreach ($set as $array)
            {
                ...
                if ( ! in_array($rule, $this->_emptyRules) && ! Valid::not_empty($value))
                {
                    continue;
                }
    ...
    public function errors($file = null, $translate = true)
    {
        ...
        foreach ($this->_errors as $field => $set)
        {
            ...
            $message = Sdk::message($file, "{$field}.{$error}");
            if ($message && is_string($message))
            {
                // Found a message for this field and error
            }
            elseif ($message = Sdk::message($file, "{$field}.default") && is_string($message))
            {
                // Found a default message for this field
            }
            elseif ($message = Sdk::message($file, $error) && is_string($message))
            {
                // Found a default message for this error
            }
            elseif ($message = Sdk::message('validation', $error) && is_string($message))
            {
                // Found a default message for this error
            }
            else
