...
use sdk\Base\Exception\HelperException;
use sdk\Base\Sdk;
...
class Url
{
    ...
    public static function detectUri()
    {
        ...
        else
        {
            ...
            $baseUrl = parse_url(Sdk::$baseUrl, PHP_URL_PATH);
            if (strpos($uri, $baseUrl) === 0)
            {
                // Remove the base URL from the URI
                $uri = (string) substr($uri, strlen($baseUrl));
            }
            if (Sdk::$indexFile && strpos($uri, Sdk::$indexFile) === 0)
            {
                // Remove the index file from the URI
                $uri = (string) substr($uri, strlen(Sdk::$indexFile));
            }
        }
    ...
    public static function base($protocol = null, $index = false)
    {
        ...
        if (true === $index && ! empty(Sdk::$indexFile))
        {
            // Add the index file to the URL
            $baseUrl .= Sdk::$indexFile . '/';
        }
