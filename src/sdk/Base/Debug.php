...
class Debug
{
    ...
    public static function enable()
    {
        ...
        set_exception_handler(['sdk\Base\Exception\BaseException', 'handler']);
        set_error_handler(['sdk\Base\Sdk', 'errorHandler']);
        ...
        register_shutdown_function(['sdk\Base\Debug', 'shutdownHandler']);
    ...
    public static function shutdownHandler()
    {
        ...
        if (in_array($error['type'], self::$shutdownErrors))
        {
            ...
            BaseException::handler(new ErrorException($error['message'], $error['type'], 0, $error['file'], $error['line']));
            exit(1);
        }
    }
    ...
    protected static function _dump(& $var, $length = 128, $limit = 10, $level = 0)
    {
        ...
        elseif (is_resource($var))
        {
            if (($type = get_resource_type($var)) === 'stream' && $meta = stream_get_meta_data($var))
            {
                ...
                if (isset($meta['uri']))
                {
                    ...
                    return '<small>resource</small><span>(' . $type . ')</span> ' . htmlspecialchars($file, ENT_NOQUOTES, Sdk::$charset);
                }
            }
            else
            {
        ...
        elseif (is_string($var))
        {
            if (strlen($var) > $length)
            {
                ...
                $str = htmlspecialchars(substr($var, 0, $length), ENT_NOQUOTES, Sdk::$charset) . '&nbsp;&hellip;';
            }
            else
            {
                // Encode the string
                $str = htmlspecialchars($var, ENT_NOQUOTES, Sdk::$charset);
            }
        ...
        elseif (is_array($var))
        {
            ...
            elseif ($level < $limit)
            {
                ...
                foreach ($var as $key => & $val)
                {
                    ...
                    if ( ! is_int($key))
                    {
                        $key = '"' . htmlspecialchars($key, ENT_NOQUOTES, Sdk::$charset) . '"';
                    }
        ...
        return '<small>' . gettype($var) . '</small> ' . htmlspecialchars(print_r($var, true), ENT_NOQUOTES, Sdk::$charset);
    }
     ...
     *     // Displays SYSPATH/classes/Sdk.php
     *     echo self::path(Sdk::findFile('classes', 'Sdk'));
     *
    ...
    public static function source($file, $lineNumber, $padding = 5)
    {
        ...
        while (false !== ($row = fgets($file)))
        {
            ...
            if ($line >= $range['start'])
            {
                ...
                $row = htmlspecialchars($row, ENT_NOQUOTES, Sdk::$charset);
